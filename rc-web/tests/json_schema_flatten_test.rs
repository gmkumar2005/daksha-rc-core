...
#[tokio::test]
async fn test_documentation_mapping_examples() {
    // Test the mapping examples from the documentation
    ...
    // Define expected mappings from documentation
    let expected_mappings = vec![
        ("name", "TEXT", "entity_data ->> 'name'"),
        ("age", "INTEGER", "(entity_data ->> 'age')::INTEGER"),
        ("salary", "NUMERIC", "(entity_data ->> 'salary')::NUMERIC"),
        ("active", "BOOLEAN", "(entity_data ->> 'active')::BOOLEAN"),
        (
            "birthdate",
            "TIMESTAMPTZ",
            "(entity_data ->> 'birthdate')::TIMESTAMPTZ",
        ),
        (
            "createdat",
            "TIMESTAMPTZ",
            "(entity_data ->> 'createdat')::TIMESTAMPTZ",
        ),
        ("email", "TEXT", "entity_data ->> 'email'"),
        ("tags", "JSONB", "entity_data ->> 'tags'"),
        ("metadata", "JSONB", "entity_data ->> 'metadata'"),
    ];

    // Verify each expected mapping
    for (expected_name, expected_type, expected_pattern) in &expected_mappings {
        let found = result
            .iter()
            .find(|attr| attr.attribute_name == *expected_name);
        ...
        assert!(
            found.is_some(),
            "Documentation example attribute '{}' not found. Available: {:?}",
            expected_name,
            result.iter().map(|a| &a.attribute_name).collect::<Vec<_>>()
        );
        ...
        assert_eq!(
            attr.column_type, *expected_type,
            "Documentation example type mismatch for '{}': expected '{}', got '{}'",
            expected_name, expected_type, attr.column_type
        );
        assert_eq!(
            attr.generated_column_pattern, *expected_pattern,
            "Documentation example pattern mismatch for '{}': expected '{}', got '{}'",
            expected_name, expected_pattern, attr.generated_column_pattern
        );
    }

    // Ensure we got exactly the expected number of attributes
    assert_eq!(
        result.len(),
        expected_mappings.len(),
        "Expected {} attributes from documentation example, got {}",
        expected_mappings.len(),
        result.len()
    );
}
...
#[test]
fn test_flatten_student_schema() {
    let schema = json!({
